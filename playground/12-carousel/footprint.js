...
let abcOrder   = {'id':'abcOrder',  'value':'abc',  'label':'ABC',  'checked':'checked' };
...
function appendControls() {
    ...
    elems.push( createOption2(scaleOpt) )
    elems.push( createOption2(chartOpt) )
    elems.push( createOption2(modeOpt) )
    elems.push( createOption2(orderOpt) )
...
function createOption2(option) {
    ...
    // <div class="btn-group" role="group1" aria-label="Basic radio toggle button group">

    buf.push("<div class=\"btn-group \"  aria-label=\"Basic radio toggle button group\" >")

    buf.push("<input id=\"" + option.name + "\" class=\"btn-check\" type=\"radio\" name=\"" + option.name  + "\" autocomplete=\"off\" disabled >");
    buf.push("<label class=\"btn btn-secondary shadow-none btn-xs\">" + option.title + "</label>");


    option.items.forEach(function (item) {
        buf.push(createOptionItem2(option.name, item))
    });
    buf.push("</div>");
...
function createOptionItem2(name, item) {
    ...
    // <input id="btnradio4"  class="btn-check"  type="radio" name="btnradio" autocomplete="off" disabled>
    // <label class="btn btn-secondary shadow-none btn-xs" for="btnradio4">Mode</label>

    buf.push("<input id=\"" + item.id + "\" class=\"btn-check\" type=\"radio\" name=\"" + name + "\" autocomplete=\"off\"" + item.checked + ">");
    buf.push("<label class=\"btn btn-secondary shadow-none btn-xs\" for=\"" + item.id + "\">" + item.label + "</label>");
